...
class InfoSet:
    ...
    def update_current_strategy(self):
        ...
        if normalization > 0:
            ...
            self.current_strategy = {action: 1 / len(self.current_strategy) for action in self.cumulative_regrets.keys()}
...
class State:
    ...
    def get_info_set(self, valid_actions) -> InfoSet:
        return InfoSet(valid_actions)

    def valid_actions(self) -> List[Action]:
        raise NotImplementedError()

    def take_action(self, action: Action, copy: bool = True):
        raise NotImplementedError()
...
class CFR:
    ...
    def _cfr(self, agent_id, state: State, agent_path_probs: List[float], depth=0):
        ...
        # Don't create an info set for states where the player has no choice
        if len(valid_actions) == 1:
            return self._cfr(agent_id, state.take_action(valid_actions[0], copy=False), agent_path_probs.copy(), depth=depth+1)

        # Get info set
        info_set_key = state.state_string()
        info_set = self.infoSet_map.get(info_set_key)
        if info_set is None:
            #print(f'Info set created for hand {info_set_key}')
            info_set = state.get_info_set(valid_actions)
            self.infoSet_map[info_set_key] = info_set
        ...
        for action in valid_actions:
            ...
            action_utils[action] = self._cfr(agent_id, state.take_action(action), new_path_probs, depth=depth+1)
